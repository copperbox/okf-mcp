...
describe("server tools", () => {
  ...
  it("get_sources falls back to a v0.1 Citations list, marking the origin", async () => {
    const result = (await callJson(client, "get_sources", { id: "tables/orders" })) as {
      ...
      stale: boolean;
      generatedAt?: string;
      sources: { id: string; resource: string; title?: string; cited: boolean }[];
    };
    assert.equal(result.origin, "citations");
    assert.equal(result.trust, "unverified");
    assert.equal(result.stale, false);
    // The fixture is a v0.1 document, so `generatedAt` comes from `timestamp`.
    assert.equal(typeof result.generatedAt, "string");
    assert.equal(result.sources.length, 3);
    assert.equal(result.sources[1]?.resource, "/tables/customers.md");
    // Nothing in a v0.1 body attributes claims by footnote.
...
describe("authoring tools", () => {
  ...
  describe("update_concept", () => {
    ...
    it("patches frontmatter and one section, logs an Update entry, and reindexes", async () => {
      ...
      assert.deepEqual(payload.updatedKeys, ["title", "generated"]);
    ...
    it("refreshes generated by default and pins it with keepGenerated", async () => {
      await writeConcept(
        root,
        "tables/stamped.md",
        { type: "Table", generated: { by: "human:x", at: "2020-01-01T00:00:00Z" } },
        "Body",
      );
      const client = await connectLocal({ writable: true });
      ...
      let concept = (await callJson(client, "get_concept", { id: "tables/stamped" })) as {
        frontmatter: Record<string, { at?: string }>;
      };
      assert.equal(concept.frontmatter.generated?.at, "2020-01-01T00:00:00Z");
      ...
      assert.ok(Date.parse(concept.frontmatter.generated?.at as string) >= before);
    });

    it("get_sources reports v0.2 signals and which entries the body cites", async () => {
      await writeConcept(
        root,
        "tables/ga4.md",
        {
          ...
          sources: [
            ...
            { id: "unused", resource: "https://example.com/other" },
          ],
        },
        "Sharded daily.[^ga4-schema]\n\n[^ga4-schema]: GA4 schema\n",
      );
      const client = await connectLocal({ writable: true });
      const result = (await callJson(client, "get_sources", { id: "tables/ga4" })) as {
        origin: string;
        trust: string;
        stale: boolean;
        sources: { id: string; cited: boolean; usage_window?: { from: string } }[];
      };
      assert.equal(result.origin, "frontmatter");
      assert.equal(result.trust, "human-reviewed");
      assert.equal(result.stale, true);
      // The shared usage_window frames every usage_count (spec §5.1).
      assert.equal(result.sources[0]?.usage_window?.from, "2026-06-01");
      // `cited` is the difference between a source the body attributes a claim
      // to and one merely listed.
      assert.deepEqual(
        result.sources.map((s) => [s.id, s.cited]),
        [
          ["ga4-schema", true],
          ["unused", false],
        ],
    ...
    it("still accepts the deprecated keepTimestamp alias", async () => {
      await writeConcept(
        root,
        "tables/aliased.md",
        { type: "Table", generated: { by: "human:x", at: "2020-01-01T00:00:00Z" } },
      ...
      assert.notEqual(pinned.isError, true);
      const concept = (await callJson(client, "get_concept", { id: "tables/aliased" })) as {
        frontmatter: Record<string, { at?: string }>;
      };
      assert.equal(concept.frontmatter.generated?.at, "2020-01-01T00:00:00Z");
    });
  ...
  describe("promote_concept", () => {
    ...
    it("moves the concept, leaves a stub, and logs and reindexes both bundles", async () => {
      ...
      assert.match(stub, /sources:\n\s+- id: promoted-copy/);
      assert.doesNotMatch(stub, /# Citations/);
