...
| `validate_bundle` | OKF v0.1 conformance errors + soft warnings (broken links, bundle-absolute leading-`/` links, dangling `../` links into colocated siblings, malformed recommended frontmatter fields, malformed or unresolved citations, duplicate top-level headings, `index.md` / `log.md` structure checks); warnings with a safe mechanical fix name their `okf-mcp repair` fixer id |
