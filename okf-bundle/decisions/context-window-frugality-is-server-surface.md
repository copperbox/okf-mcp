...
- **Search hits point into sections**: `section`/`matchedSections` on body matches feed [`get_concept`'s `sections`/`section` arguments](../architecture/search-scoring.md). Since 2.1.0 hits also carry a `recommendedRead` of `sections` or `full`, because section-only guidance made agents fetch whole documents one section per call; the coverage counts it is computed from stay behind `detail: "full"`, like the other per-hit diagnostics.
- **`get_concept` offers graduated reads**: `outline: true` (shape only), `section` (one subtree), `sections` (several subtrees in one call), full body — see the [server surface](../architecture/mcp-server.md).
