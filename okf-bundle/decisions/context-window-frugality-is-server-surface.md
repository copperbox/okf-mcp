...
- **Search hits point into sections**: `section`/`matchedSections` on body matches feed [`get_concept`'s `sections`/`section` arguments](../architecture/search-scoring.md). Since 2.1.0 hits also carry read coverage and a `recommendedRead` of `sections` or `full`, because section-only guidance made agents fetch whole documents one section per call.
