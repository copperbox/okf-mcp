...
| `get_concept` | One document: frontmatter, body, section headings. Prefer partial reads: `section` returns one heading's subtree, `outline: true` returns the section list (heading, level, content size) without the body. The outgoing-link offset arrays (`links`, `frontmatterLinks`) return only with `detail: "full"` |
