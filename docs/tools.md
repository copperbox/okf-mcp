...
| Tool | Purpose |
|---|---|
| `list_bundles` | Configured bundles with concept and reserved-file counts, read-only flags, declared `description`s, and a `loaded` marker for [lazily mounted](colocated-bundles.md#lazy-mounting) bundles |
...
| `list_concepts` | Concept metadata, filterable by prefix/type — the whole-catalog enumeration, paginated (`limit` default 50, max 500; `offset`) with a `total` count; `search_concepts` is the entry point for finding relevant concepts |
| `get_concept` | One document: frontmatter, body, section headings. Read individual sections when one or a small subset is matched; read the full concept when every section matched or the selected sections contain most of the document (70% of its characters). `sections` returns several headings' subtrees in one call as `sectionContents` (document order, every heading sharing a requested name, a nested section folded into a requested parent), `section` returns one, `outline: true` returns the section list (heading, level, content size) without the body. The outgoing-link offset arrays (`links`, `frontmatterLinks`) return only with `detail: "full"` |
...
| `read_document` | Raw markdown of any document, including reserved files — for concepts prefer `get_concept` outline/section reads; `startLine`/`endLine` (1-based, inclusive) return a slice with a `totalLines` count; a missing `index.md` is synthesized (`synthesized: true`) |
| `search_concepts` | Keyword query + type/tag/path/link/orphan filters and the v0.2 lifecycle/trust filters (`status`, `minTrust`, `stale`), paginated (default 10/page). Hits are concise by default; `detail: "full"` adds `score`, `matchedIn`, each hit's `status`/`trust` tier plus `stale: true` when past `stale_after`, and the read-coverage counts (`matchedSectionCount`, `sectionCount`, `matchedCharacters`, `documentCharacters`) behind `recommendedRead`. All-keyword hits rank first with a fallback to any-keyword (`termMatching: "any"`); low scorers under a quarter of the top score are dropped into `omitted`; zero hits return `tagHints`; an exact-`resource` filter maps an asset URI to its concept. Body-matched hits name the enclosing `section` (and `matchedSections` when several sections matched, or when the match sits before the first heading), which feed `get_concept`'s `sections` / `section` arguments, and `recommendedRead`: `full` when every section matched or the matched sections hold 70%+ of the document, otherwise `sections` |
...
| `graph_summary` | Counts, types, tags, orphans (first 25 plus an `orphanCount`; the full list via `search_concepts` with `orphanOnly: true`), derived `crossBundleEdges` |
| `get_neighbors` | Bounded expansion around a concept, capped at 50 nodes (a truncated result carries a `note` suggesting a lower depth); nodes are `id` + `title` + `type` unless `detail: "full"` adds `bundle`/`path`/`description`/`tags`; `crossBundle: true` follows [derived edges](cross-bundle.md) |
| `find_path` | Shortest directed path between concepts; `crossBundle: true` accepts `bundle:concept` IDs |
| `export_graph` | Graph as `json` / `dot` / `mermaid`; `crossBundle: true` exports one namespaced graph. For `json` a `detail` ladder applies: `summary` (default) returns counts by type/kind plus top-degree hubs, `ids` returns bare node ids and `[from, to(, kind)]` edge tuples, `full` returns complete nodes/edges — `ids`/`full` cap at 300 nodes / 600 edges with a truncation `note`; `dot`/`mermaid` are uncapped and can be large |
